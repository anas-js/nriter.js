/*!
 * nriter.js v1.0.2 (https://github.com/nonehub/nriter.js)
...
function writing(selector, text, speed, waiting, value) {
  ...
  for (let len = 0; len < document.querySelectorAll(selector).length; len++) {
      ...
      speed = document
          ...
          [len].getAttribute("data-speed");
      waiting = document
          .querySelectorAll(selector)
          [len].getAttribute("data-waiting");
      value = document
          .querySelectorAll(selector)
          [len].getAttribute("data-value");
      if (text == null) {
      ...
      let removeText = setInterval(function removeTextFunction() {
          // Cut the last letter of the word and print the word after editing
          document.querySelectorAll(`${selector}`)[
              len
          ].childNodes[0].textContent = document
              .querySelectorAll(`${selector}`)
          ...
          if (
              ...
              let addText = setInterval(function () {
                  ...
                  document.querySelectorAll(`${selector}`)[
                      len
                  ].childNodes[0].textContent = arrayNowText.substr(
                      0,
                      numberLength
                  );
                  numberLength++;
                  ...
                  if (
                      arrayNowText.length ==
                      document.querySelectorAll(`${selector}`)[len]
                          .childNodes[0].textContent.length
                  ) {
                      // Stop letter writing
